...
export const sendMediaGroupContent = async (bot, chatId, content, options = {}) => {
    if (content.text) {
        return await bot.sendMessage(chatId, safeMarkdownV2(content.text), { parse_mode: "MarkdownV2", ...options });
    } else if (content.media) {
        if (content.media.photo) {
            return await bot.sendPhoto(chatId, content.media.photo[content.media.photo.length - 1].file_id, {
                caption: safeMarkdownV2(content.media.caption || ""),
        ...
        const mediaArray = content.mediaGroup.map(m => {
            ...
            return { type, media, caption: safeMarkdownV2(m.caption || ""), parse_mode: "MarkdownV2" };
        });
